...
// ---------------------------------------------------------------------------
// 4 — the document lane: the whole PDF in one call (2026-09-01)
//
// The text/vision split existed for cover sheets and for cost. Measured on the
// same 45 benchmark documents the single document read equals or beats the
// text lane on nearly every field (groundlevel 100/100 vs 94/80, quality
// 93/93 vs 92/86) and the feared table-digit misreads did not appear. Routing
// remains only as guards: large-format sheets (tiled drawing read), files over
...
export async function extractDocument(pdfPath: string): Promise<FieldRead[]> {
